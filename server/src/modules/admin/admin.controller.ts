...
import { sendApprovalEmail, sendRejectionEmail } from '../../shared/utils/email.service.js';
...
export const getAnalytics = async (req: any, res: Response) => {
    try {
        ...
        res.json({
            counts: {
                totalUsers, totalGraduates, totalEmployers, totalJobs,
                activeJobs, totalApplications, hiredApplications,
                rejectedApplications, pendingApplications, appliedApplications,
            },
            recentUsers,
            jobCategories,
        });
...
export const getPendingEmployers = async (req: any, res: Response) => {
    try {
...
export const approveEmployer = async (req: any, res: Response) => {
    try {
        const employer = await User.findById(req.params.id);
        if (!employer) { res.status(404).json({ message: 'Employer not found' }); return; }
        if (employer.role !== 'employer') { res.status(400).json({ message: 'User is not an employer' }); return; }

        employer.verified = true;
        (employer as any).approvalStatus = 'approved';
        (employer as any).rejectionReason = '';
        await employer.save();
        ...
        res.json({
            ...
            employer: {
                ...
                approvalStatus: (employer as any).approvalStatus,
            },
        });
    } catch (error: any) { res.status(500).json({ message: error.message }); }
};

export const rejectEmployer = async (req: any, res: Response) => {
    try {
        ...
        (employer as any).approvalStatus = 'rejected';
        (employer as any).rejectionReason = reason || 'Your application did not meet our requirements.';
        await employer.save();

        sendRejectionEmail(employer.email, employer.fullName, (employer as any).rejectionReason)
            .catch(err => console.error('Email failed:', err.message));
        ...
        res.json({
            ...
            employer: {
                ...
                companyName: employer.companyName,
                approvalStatus: (employer as any).approvalStatus,
                rejectionReason: (employer as any).rejectionReason,
            },
        });
    } catch (error: any) { res.status(500).json({ message: error.message }); }
};
...
export const getAllJobs = async (req: any, res: Response) => {
    try {
        const jobs = await Job.find().populate('company', 'companyName email').sort({ createdAt: -1 });
        res.json(jobs);
    } catch (error: any) { res.status(500).json({ message: error.message }); }
};
...
export const getReports = async (req: any, res: Response) => {
    try {
        const [users, jobs, applications, faqs, jobFaqs, employerSettings] = await Promise.all([
            ...
            JobFAQ.find().populate('employer', 'fullName companyName').populate('job', 'title'),
...
export const updateJobFAQ = async (req: any, res: Response) => {
    try {
        ...
        if (jobFAQ) {
            ...
            if (jobFAQ.job) await (jobFAQ as any).populate('job', 'title');
            res.json(jobFAQ);
        } else { res.status(404).json({ message: 'Job FAQ not found' }); }
    } catch (error: any) { res.status(500).json({ message: error.message }); }
};
...
export const getAllEmployerSettings = async (req: any, res: Response) => {
    try {
        const settings = await EmployerSettings.find().populate('user', 'fullName email companyName');
        res.json(settings);
    } catch (error: any) { res.status(500).json({ message: error.message }); }
};
...
export const createEmployerSettings = async (req: any, res: Response) => {
    try {
        ...
        if (existing) { res.status(400).json({ message: 'Settings for this employer already exist' }); return; }
...
export const createUser = async (req: any, res: Response) => {
    try {
        ...
        if (role === 'employer') {
            ['degree', 'university', 'major', 'graduationYear', 'jobPreferences', 'skills', 'experiences', 'internships', 'education', 'awards', 'certifications', 'projects', 'languages'].forEach(f => delete userData[f]);
        }
...
export const uploadImage = async (req: any, res: Response) => {
    ...
    } catch (error: any) { res.status(500).json({ message: 'Error uploading image' }); }
};
