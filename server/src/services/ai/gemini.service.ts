...
class GeminiService {
  ...
  async analyzeJobSuitability(
    ...
    - Verified Assessments (Skill, Score, Category Scores): {verifiedSkills}
    ...
    1. Skills Match (Max 40 points): 
       ...
       - Give BONUS points if they have Verified Assessments with high scores for the required skills.
    ...
    try {
      const input = await prompt.format({
        ...
        verifiedSkills: JSON.stringify(
          userProfile.verifiedSkills?.map(v => {
            const catScores = v.categoryScores ? Object.entries(v.categoryScores).map(([k, val]) => `${k}: ${val}`).join(', ') : '';
            return `${v.skill} (Score: ${v.score}${catScores ? `, Categories: [${catScores}]` : ''})`;
          }) ?? []
        ),
      });
