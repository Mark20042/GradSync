...
const ChatLauncher = ({ isOpen, onClick, showHint, onDismissHint }) => (
  <div className="fixed bottom-24 right-4 sm:bottom-6 sm:right-6 z-[200] flex flex-col items-end gap-3">
    ...
    <motion.button
      ...
      className={`relative h-[46px] sm:h-[60px] rounded-full flex items-center justify-center overflow-hidden transition-all ${
        isOpen
          ? "w-[46px] sm:w-[60px] bg-sky-500 text-white shadow-xl"
          : "px-1.5 pr-3.5 sm:px-2 sm:pr-5 bg-sky-400 shadow-[0_8px_25px_rgba(56,189,248,0.35)] hover:shadow-[0_12px_30px_rgba(56,189,248,0.45)] hover:-translate-y-1"
      }`}
    >
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          {isOpen ? (
           <div className="relative w-[32px] h-[32px] sm:w-[44px] sm:h-[44px] flex-shrink-0 rounded-full flex items-center justify-center ">
                <img
                  src="/qubi.svg?v=4"
                  alt="Qubi"
                  className="w-[85%] h-[85%] object-contain"
                />
                <span className="absolute bottom-0 right-0 w-2 h-2 sm:w-3 sm:h-3 bg-emerald-500 border-2 border-white rounded-full z-10" />
              </div>
          ) : (
            <>
              <div className="relative w-[32px] h-[32px] sm:w-[44px] sm:h-[44px] flex-shrink-0 flex items-center justify-center ">
                <img
                  src="/qubi.svg?v=4"
                  alt="Qubi"
                  className="w-[85%] h-[85%] object-contain"
              ...
              <span className="font-semibold text-[13px] sm:text-[15px] text-white whitespace-nowrap drop-shadow-sm ml-0.5 sm:ml-0">
                Ask Bibo
              </span>
            </>
          )}
