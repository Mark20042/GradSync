    ...
    <div className="space-y-4 mt-6">
      {rankedApps.map((application, index) => (
        ...
        >
          ...
          {/* Actions */}
          <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 mt-4 md:mt-0 sm:pl-16 md:pl-0 w-full md:w-auto">
            <div className="col-span-2 sm:col-auto flex justify-start sm:mr-2">
              <StatusBadge status={application.status} />
            </div>

            <button
              className="flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 hover:text-blue-700 transition-all shadow-sm active:scale-95"
              title="Download Resume"
              onClick={() => handleDownloadResume(application.applicant.resume)}
            >
              <Download className="w-5 h-5" />
              <span className="sm:hidden text-sm font-medium">Resume</span>
            </button>

            <button
              className="flex items-center justify-center gap-1.5 px-3 py-2 bg-gray-900 text-white text-sm font-semibold rounded-lg hover:bg-gray-800 transition-all shadow-md shadow-gray-200 active:scale-[0.98] cursor-pointer"
