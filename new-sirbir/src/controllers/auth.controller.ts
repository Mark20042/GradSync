...
import { sendVerificationSuccessEmail, sendVerificationFailedEmail } from "@/utils/email.service.js";
import fs from "fs";
  ...
  try {
    ...
    if ((req as any).files) {
      ...
      if (files.avatar && files.avatar[0]) {
        const result = await uploadToCloudinary(files.avatar[0].buffer, "gradsync/avatars", "image", files.avatar[0].originalname);
        avatarUrl = result.url;
      }
      if (files.tor && files.tor[0]) {
        const result = await uploadToCloudinary(files.tor[0].buffer, "gradsync/documents", "image", files.tor[0].originalname);
        torUrl = result.url;
      }
      if (files.businessPermit && files.businessPermit[0]) {
        const result = await uploadToCloudinary(files.businessPermit[0].buffer, "gradsync/documents", "image", files.businessPermit[0].originalname);
    ...
    const runBackgroundVerification = async () => {
      try {
        ...
        if (isEmployer && (req as any).files && (req as any).files.businessPermit) {
          const file = (req as any).files.businessPermit[0];
          const tempPath = bufferToTempFile(file.buffer, file.originalname);
          console.log(`🔍 Running OCR verification on Business Permit for ${fullName}... (${file.originalname})`);
          ocrResult = await verifyDocument(tempPath, "businessPermit");
          fs.unlinkSync(tempPath); // cleanup temp file
        } else if (!isEmployer && (req as any).files && (req as any).files.tor) {
          const file = (req as any).files.tor[0];
          const tempPath = bufferToTempFile(file.buffer, file.originalname);
          console.log(`🔍 Running OCR verification on TOR for ${fullName}... (${file.originalname})`);
        ...
        if (ocrResult.verified) {
          ...
          sendVerificationSuccessEmail(user.email, user.fullName, user.role).catch(
          ...
          if (avatarUrl) {
            ...
            if (pid) { cleanups.push(deleteFromCloudinary(pid, "image")); console.log(`🧹 Deleting avatar from Cloudinary: ${pid}`); }
          }
          if (torUrl) {
          ...
          if (businessPermitUrl) {
            ...
            if (pid) {
              cleanups.push(deleteFromCloudinary(pid, "image"));
              console.log(`🧹 Deleting Business Permit from Cloudinary: ${pid}`);
            }
          }
          await Promise.allSettled(cleanups);
          await User.findByIdAndDelete(user._id);
          console.log(`✅ User ${user.email} and all Cloudinary files deleted.`);
          sendVerificationFailedEmail(user.email, user.fullName, user.role).catch(
            (err) => console.error(err),
          );
        }
      } catch (err: any) {
...
const login = async (
  ...
  try {
    ...
    if (user.password !== password) throw new UnauthenticatedError("Invalid email or password");
    ...
    res.status(StatusCodes.OK).json({
      ...
      resume: user.resume || "", isAdmin: user.isAdmin, verified: user.verified, isProfileComplete,
    });
  } catch (error) {
    next(error);
  }
...
const uploadImage = async (
  ...
  try {
    ...
    const result = await uploadToCloudinary(req.file.buffer, "gradsync/avatars", "image", req.file.originalname);
    ...
    res.status(StatusCodes.OK).json({ imageUrl, avatarUrl: imageUrl, message: "Avatar updated successfully" });
  } catch (error) {
    next(error);
  }
};
...
const uploadResume = async (
  ...
  try {
    ...
    const result = await uploadToCloudinary(req.file.buffer, "gradsync/resumes", "image", req.file.originalname);
    ...
    if (currentUser?.resume) {
      ...
      if (previousPublicId) await deleteFromCloudinary(previousPublicId, "image");
    }
    ...
    res.status(StatusCodes.OK).json({ resumeUrl, message: "Resume updated successfully" });
  } catch (error) {
    next(error);
  }
};

export { register, login, logout, getMe, setupProfileGrad, uploadImage, uploadResume };
