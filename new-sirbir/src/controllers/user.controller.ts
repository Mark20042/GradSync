...
import { deleteFromCloudinary, getPublicIdFromUrl } from "@/services/cloudinary.service.js";
...
const updateProfile = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (user.role === "employer") {
      ...
      if (body.companyLogo && body.companyLogo !== user.companyLogo && user.companyLogo) {
...
const deleteProfile = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const user = await User.findById(req.user._id);
    if (!user) throw new NotFoundError("User not found");
    ...
    res.status(StatusCodes.OK).json({ message: "Account and all associated data deleted successfully" });
  } catch (error) { next(error); }
};

const deleteResume = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const user = await User.findById(req.user._id);
    if (!user) throw new NotFoundError("User not found");
    if (user.role !== "graduate") throw new UnauthorizedError("Only jobseekers can delete resumes");
...
const getEmployers = async (_req: Request, res: Response, next: NextFunction) => {
  try {
...
export { updateProfile, deleteProfile, deleteResume, getEmployers, getUserById };
