...
const checkAndSendAutoReply = async (
  recipientId,
  senderId,
  messageContent,
  conversationId,
  ...
  try {
    ...
    console.log(
      `[AutoReply] Checking FAQs for recipient ${recipientId}. Found ${faqs.length} FAQs.`
    );
    ...
    for (const faq of faqs) {
      ...
      const messageWords = lowerContent
        .split(/\s+/)
      ...
      if (messageWords.length >= 2) {
        ...
        const matchingWords = questionWords.filter((qWord) =>
          messageWords.some(
            (mWord) => mWord.includes(qWord) || qWord.includes(mWord)
          )
        );

        // Require at least 50% of question keywords to match
        const matchRatio =
          matchingWords.length / Math.max(questionWords.length, 1);
      ...
      if (!match && lowerContent.length >= 10) {
        const exactMatch =
          lowerContent.includes(questionLower) ||
          questionLower.includes(lowerContent);
    ...
    if (!isFaqMatch) {
      ...
      if (todayHours) {
        if (!todayHours.isOpen) {
          ...
          console.log(
            `   - Business hours: ${todayHours.start} - ${todayHours.end}`
          );
          console.log(`   - Is offline? ${isOffline}`);
        }
      } else {
        console.log(
          `   - No business hours set for ${dayName}, defaulting to offline`
        );
      }
      ...
      if (isOffline && settings.autoReplyMessage) {
        ...
        console.log(
          `[AutoReply] Sending offline message: "${autoReplyContent}"`
        );
      }
    }
    ...
    if (io) {
      const populatedReply = await Message.findById(savedReply._id).populate(
        "sender",
        "fullName avatar"
      );
