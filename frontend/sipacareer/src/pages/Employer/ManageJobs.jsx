...
const ManageJobs = () => {
  ...
  const [sortDirection, setSortFirection] = useState("asc");
  ...
  const handleSort = (field) => {
    if (sortField === field) {
      setSortFirection(sortDirection === "asc" ? "desc" : "asc");
    } else {
      setSortField(field);
      setSortFirection("asc");
    }
  };
  ...
  const handleStatusChange = async (jobId) => {
    try {
      const response = await axiosInstance.put(
        API_PATH.JOBS.TOGGLE_CLOSE(jobId)
      );
  ...
  const handleDeleteJob = async (jobId) => {
    try {
      await axiosInstance.delete(API_PATH.JOBS.DELETE_JOB(jobId));
  ...
  //decide which icon to display based on current sort field and direction
  ...
  const getPostedJobs = async (disableLoader) => {
    ...
    try {
      const response = await axiosInstance.get(API_PATH.JOBS.GET_JOBS_EMPLOYER);
