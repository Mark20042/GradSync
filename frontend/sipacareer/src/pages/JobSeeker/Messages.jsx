...
const Messages = () => {
    ...
    if (conversationId) {
      const fetchData = async () => {
        ...
        try {
          ...
          if (recipient && recipient.role === "employer") {
            const faqResponse = await axiosInstance.get(API_PATH.EMPLOYER.GET_PUBLIC_FAQS(recipient._id));
  ...
  const handleSendMessage = (e) => {
    e.preventDefault();
    if (!socket || newMessage.trim() === "" || !recipient) return;
              ...
              <div
                onClick={() => {
                  if (recipient?.role === 'employer') {
                    navigate(`/company/${recipient._id}`);
                  } else {
                    navigate(`/user/${recipient._id}`);
                  }
                }}
                className="flex items-center gap-3 cursor-pointer hover:bg-gray-50 rounded-xl p-2 -m-2 transition-colors group"
              >
                <div className="relative">
                  {recipient?.avatar || recipient?.companyLogo ? (
                    <img
                      src={recipient.avatar || recipient.companyLogo}
                      alt={recipient.fullName || recipient.companyName}
                ...
                <div>
                  <h2 className="font-bold text-gray-900 text-base leading-tight group-hover:text-blue-600 transition-colors">
                    {recipient?.companyName || recipient?.fullName || "User"}
                  </h2>
                  <div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium mt-0.5">
                    <Briefcase className="w-3 h-3" />
                    <span className="truncate max-w-[200px]">{job?.title || "General Inquiry"}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {faqs.length > 0 && (
              <div className="mb-4 flex gap-2 overflow-x-auto pb-2 no-scrollbar mask-linear-fade">
                {faqs.filter(faq => {
                  ...
                  if (job && faq.job._id === job._id) return true;
                  return false;
                }).map((faq) => (
                  <button
                    key={faq._id}
